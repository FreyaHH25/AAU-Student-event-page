...
if (editEventId) {
  ...
  document.querySelector(".title_with_icon").innerHTML = `
    ...
    Edit Event`;
  document.querySelector(".create_description").innerText =
  ...
  try {
    ...
    if (eventData.visibility) {
      selectedVisibility = eventData.visibility.map((val) => {
        const option = Array.from(visibilitySelect.options).find(
          (opt) => opt.value === val,
        );
...
const visibilityTagsDisplay = document.getElementById(
  "visibility-tags-display",
);
...
visibilitySelect.addEventListener("change", function () {
  ...
  if (
    selectedValue &&
    !selectedVisibility.some((v) => v.val === selectedValue)
  ) {
  ...
  .addEventListener("submit", async function (event) {
    ...
    const newEventData = {
      ...
      imageUrl:
        ...
        "images/aau-entrance.png",
    };

    const result = validateEvent(newEventData);

    if (!result.valid) {
      alert(result.errors.join("\n"));
      return; // stop â€” send ikke til backend
    }
